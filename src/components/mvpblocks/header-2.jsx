...
// Smooth scroll handler for landing search button bug #110
  const handleSearchClick = (e) => {
    ...
    // Advanced Input selector fallback strategy (No alerts, pure silent detection)
    const mainSearchBar = 
      document.querySelector('input[placeholder*="Search for organizers"]') || 
    ...
    if (mainSearchBar) {
      ...
      // Auto focus setup post animation transition
      setTimeout(() => {
        mainSearchBar.focus();
      }, 500);
    } else {
  ...
  useEffect(() => {
    if (location.pathname === "/") {
      ...
      if (params.get("focusSearch") === "true") {
        setTimeout(() => {
          const mainSearchBar = document.querySelector('input[placeholder*="Search for organizers"]');
          if (mainSearchBar) {
            mainSearchBar.scrollIntoView({ behavior: "smooth", block: "center" });
            mainSearchBar.focus();
          }
