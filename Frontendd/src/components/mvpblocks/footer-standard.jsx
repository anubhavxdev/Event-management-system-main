        ...
        <div className="container m-auto grid grid-cols-1 gap-12 py-12 md:grid-cols-2 lg:grid-cols-5">
          <div className="space-y-6 lg:col-span-2">
            ...
            <p className="text-muted-foreground max-w-md">
              Building innovative solutions for modern campus communities.
            </p>
            ...
            <div className="flex gap-2">
              {data().socialLinks.map(({ icon: IconComponent, label, href }) => (
                <Button
                  ...
                  variant="outline"
                  asChild
                  className="!border-primary/30 cursor-pointer shadow-none transition-all duration-500 hover:scale-110 hover:-rotate-12 hover:bg-primary hover:text-white hover:shadow-md dark:hover:bg-primary"
                >
                  <a href={href}>
          ...
          <div className="grid w-full grid-cols-2 items-start justify-between gap-8 px-5 lg:col-span-3">
            {['product', 'company', 'resources', 'legal'].map((section) => (
              <div key={section} className="space-y-3">
                <h4 className="text-xs font-semibold uppercase tracking-wide text-slate-900 dark:text-slate-100">
                  ...
                  {data().navigation[section].map((item) => (
                    <li key={item.name}>
                      <Link
                        ...
                        className="cursor-pointer text-xs text-slate-600 transition-colors hover:text-rose-600 dark:text-slate-300"
                      >
                        {item.name}
                      </Link>
                    </li>
